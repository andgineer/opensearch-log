...
def cloudwatch():
    ...
    if json_log._logger is not None:
        handlers_to_remove = [
            handler for handler in json_log._logger.handlers if isinstance(handler, BaseHandler)
        ]
...
class MockLogRecord(logging.LogRecord):
    def __init__(
        self, name, level, filename, lineno, message, args, exc_info, func=None, sinfo=None
    ):
...
class MockOpenSearchClient:
    ...
    def bulk(self, *args, **kwargs):
        ...
        return {"took": 1, "errors": False, "items": response_items}
...
def opensearch_handler():
    ...
    with patch(
        ...
        return_value=MockOpenSearchClient(),
    ):
